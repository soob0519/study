...
%>
	<script>
		alert("로그인 후 이용해 주세요.");				// 팝업메시지 출력	
		location = "login/loginWrite.jsp";			// 페이지 이동 처리
	</script>
<%
	return;
}
...
<%
String sql2 = "SELECT COUNT(*) FROM board2 WHERE userid = '"+SessionUserid+"' AND unq = '"+unq+"'";
...
<%
String sql3 = "UPDATE board2 SET hits = hits+1 WHERE userid = '"+SessionUserid+"' AND unq = '"+unq+"'";
...
String sql = "SELECT title "
		   ...
		   +" 		,lev "
		   +" FROM board2 " 
		   +" WHERE userid = '"+SessionUserid+"' AND unq='"+unq+"'";
		...
		<table class="table_member">
			...
			<tr>
				...
				<td style="padding-left: 5px; text-align:left;">
					...
					for(int i=0; i<lev; i++){
					%>
						<img src="../../images/goldStar.JPG" width="16" height="20">
					<%
					}
					for(int i=0; i<(5-lev); i++){
					%>
						<img src="../../images/silverStar.JPG" width="16" height="20">
					<%
					}
					%>
				</td>

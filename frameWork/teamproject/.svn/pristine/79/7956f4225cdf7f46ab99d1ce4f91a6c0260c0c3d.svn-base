...
String sql =" SELECT DAY,COUNT(DAY) AS CNT1 FROM ("
		   ...
		   +" FROM SCHEDULE WHERE USERID='"+SessionUserid+"' "
...
<html lang="en">
 <head>
  ...
  <link rel="stylesheet" href="../../css/scheduleList.css"/>  <!-- 외부 css파일 불러오는 태그 -->
 </head>
		...
		<div>
			<table>
				<tr>
					...
					<th><a href="scheduleList.jsp">예약현황</a></th>
					<th><a href="boardList.jsp?code=1">리뷰</a></th>
					<th><a href="boardList.jsp?code=2">Q&A</a></th>
					<%
...
<section>
	...
	<table class="table1" style="justify-self: center;">
		<tr>
...
//			int dd2 = tt.indexOf(dd1);
			...
			<%
				...
				if(map.get(d2) != null){
					out.print("<br><a href='scheduleList.jsp?year="+yy+"&month="+mm3+"&day="+d2+"'>"+map.get(d2)+"건</a>");
				}
			%>
			</td>	
			<%
	...
	<br>
		...
		<form name="frm4" method="post" action="scheduleSave.jsp">
		<table class="table2">
	  		<tr>
				<td>예약번호</td>
				<td><%=unq %></td>

 ...
 <body>
	...
	<section>
		...
		<table class="table_member">
			...
			%>
				<button type="button" class="button4" onclick="location='boardDetailST.jsp?code=<%=code %>&unq=<%=bef_unq%>'">이전글</button>
			<%
			}
			%>
			<%
			...
			%>
				<button type="button" class="button4" onclick="location='boardDetailST.jsp?code=<%=code %>&unq=<%=nex_unq%>'">다음글</button>
			<%
			}
			%>
			</div>
			...
			if(code.equals("2")){ 
			%>
			<form name="frm" method="post" action="boardReSaveST.jsp?code=<%=code%>">				

...
// 댓글 출력을 위한 SQL ↓
String sql5 = "SELECT unq,content,to_char(rdate,'yy.mm.dd') AS rdate,lev "
			+ " 	FROM board2sub "
...
<html lang="en">
 <head>
  <meta charset="UTF-8">
  <title><%=msg %></title>
  <link rel="stylesheet" href="myPageBoard.css?after"/>
 </head>
...
body > section > div.div_agrees > form > table > tbody > tr:nth-child(2) > td > input.inp2::placeholder {
 ...
 <body>
	...
	<section>
		...
		<div class="div_agrees" style="height:720px;">
...
//					}
			...
			%>
				<button type="button" class="button4" onclick="location='boardDetail.jsp?unq=<%=bef_unq%>'">이전글</button>
			<%
			}
			%>
			<%
			...
			%>
				<button type="button" class="button4" onclick="location='boardDetail.jsp?unq=<%=nex_unq%>'">다음글</button>
			<%
			}
			%>
			</div>
			...
			<form name="frm" method="post" action="boardReSave.jsp">				
		...
		<div>
			<table style="width:840px; margin-left:40px; margin-top:20px;">
				<tr>
					<td width="10%" style="text-align:center;">순번</td>
					<td width="70%" style="text-align:center;">내용</td>
				</tr>
			<%
			int cnt2 = 0;
			int hap = 0;
			...
			%>
				<tr>
					...
					<td style="padding: 0px;"><a href="javascript:fn_action('<%=unq2%>','<%=content2%>','<%=lev%>')"><%=content2 %></a></td>
				</tr>
			<%	
			}
			if(cnt2 == 0) {

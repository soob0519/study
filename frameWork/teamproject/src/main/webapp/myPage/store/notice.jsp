...
String sql1 = "SELECT count(*) FROM storenotice ";		// 총 데이터 개수 출력
...
<html lang="en">
 ...
 <script>
	...
	function fn_btnDel(unq) {
		let result = confirm("선택한 공지를 삭제 하시겠습니까?"); 
			if(result == false){
				return false;
			} else if(result == true) {
				$.ajax({ 
					type : "post",
					url  : "noticeDelete.jsp",
					data : "unq="+unq,
					datatype : "text",
...
String sql ="SELECT B.* FROM( "
			...
			+"		SELECT "
			+"				UNQ, STID,GUBUN,TITLE,NTCONTENT,TO_CHAR(RDATE,'YYYY-MM-DD') RDATE "
			+" 			FROM STORENOTICE  WHERE STID='"+SessionUserid+"' "
...
%>
		...
		<!-- 가게 공지사항 게시판 테이블 -->
		<div class="table1">
		<form name="frm1" method="post" action="noticeSave.jsp">
			<table class="table1">
				<tr>
					<td colspan="4">
						공지사항 입력
				...
				<tr>
					<td>내용</td>
					<td colspan="2"><textarea name="ntcontent"></textarea></td>
				</tr>
			</table>
		</form>
			...
			<table class="table1">
				...
				<tr>
					...
					<td><%=gubun %></td>
					<td><%=title %></td>
					<td rowspan="2"><%=rdate %></td>
					<td rowspan="2"><button type="button" onclick="fn_btnDel('<%=unq%>');">삭제</button></td>
				</tr>	
				<tr>

...
<html lang="en">
 ...
 <body>
	...
	<nav>
		<!-- Top Menu Start -->
		<%@ include file = "../../include/boardTopMenuST.jsp" %>
		<!-- Top Menu End -->
	</nav>
	...
	<section>
		...
		<table class="table_member">
			...
			<tr>
				<td><%=rownum %></td>
				<td><%=msg %></td>
				<td style="text-align:left;"><a href="boardDetailST.jsp?code=<%=code %>&unq=<%=unq%>"><%=title %></a>(<%=cnt %>)</td>
				<td><%=writer %></td>
				<td><%=rdate %></td>
				<td><%=hits %></td>
			</tr>
			...
			<a href="boardListST.jsp?code=<%=code %>&page=1&<%=search %>" class="num first"> 《 </a>
			<a href="#" class="num bef"> 〈 </a>
			<%
			for(int p=1; p<=lastPage; p++){
			%>
				<a href="boardListST.jsp?code=<%=code %>&page=<%=p %>&<%=search %>" class="num"><%=p %></a>
			<%
//				out.print("<a href='boardList.jsp?page="+p+"' class=\"num\"> "+p+" </a>");
			}
			%>
			<a href="#" class="num bef"> 〉</a>
			<a href="boardListST.jsp?code=<%=code %>&page=<%=lastPage %>&<%=search %>" class="num last"> 》</a>
